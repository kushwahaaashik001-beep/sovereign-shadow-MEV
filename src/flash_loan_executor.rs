...
impl FlashLoanExecutor {
    ...
    pub async fn simulate_and_execute(&self, opp: &Opportunity) -> Result<B256, MEVError> {
        ...
        let survival_multiplier = if current_balance > scavenger_threshold {
        ...
        if top_sim.profit * U256::from(10) <= adjusted_total_cost * survival_multiplier {
            return Err(MEVError::SimulationFailed(format!(
                "Survival Rule Veto: Profit {} < {}x Gas {}", top_sim.profit, survival_multiplier.to::<u64>() as f64 / 10.0, adjusted_total_cost
            )));
        }

        if net_profit <= U256::ZERO || net_profit < self.min_profit_wei {
            return Err(MEVError::SimulationFailed("Profit below minimum".into()));
        }
        ...
        let tx_request = TransactionRequest::default()
            ...
            .with_gas_limit((top_sim.gas_used + 50_000) as u128) // Bug Fix: expected u128
        ...
        if broadcast_results.iter().any(|r| r.is_ok()) {
            info!("✅ [TX] Bundle accepted! Recording success for calibration.");
            ...
            if err_msg.contains("0x937c4424") || err_msg.to_lowercase().contains("zerolossshield") {
                warn!("🛡️ [SHIELD] Atomic Revert Triggered on-chain for opp={}. Slippage or Predator frontrun prevented loss.", opp.id);
                self.circuit_breaker.record_failure(FailureType::Slippage);
            } else {
                error!("❌ [TX] All relays rejected the bundle. Opening circuit breaker.");
    ...
    fn build_calldata_with_lender(&self, opp: &Opportunity, lender: Lender) -> Result<Vec<u8>, MEVError> {
        ...
        let path = crate::models::ArbitragePath {
            ...
            loan_token:  opp.input_token,
            loan_amount: opp.input_amount,
            lender,
        };
        ...
        let call = IShadowBot::executeArbitrageCall {
            loanToken:  opp.input_token,
            loanAmount: opp.input_amount,
            pathData:   path_data,
            minProfit:  self.min_profit_wei,
        };
